...
> **Activation Condition:** This agent is activated only in projects with defined sprints, multiple parallel deliveries, or large backlogs. In smaller or solo projects, CEO absorbs this role. Activation must be declared in `PERSISTENCE://PROJECT §5`.

## Responsibilities
- Manage `TASKS.md` backlog: prioritization, status, estimated vs. actual time tracking.
- Monitor delivery cadence against `PERSISTENCE://ROADMAP` milestones.
